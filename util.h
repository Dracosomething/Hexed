//
// Created by Draco on 12/18/2025.
//
...
char **splitstr(char *string, int len) {
  ...
  char **strings = (char **)malloc(final_size * sizeof(char *));
  ...
  for (int i = 0; i < str_length; i++) {
    ...
    if (current_cursor == 0) {
      strings[strings_cursor] = (char *)malloc(len * sizeof(char) + 1);
    }
...
int starts_with(char *string, char check) { return string[0] == check; }

int islower_custom(char to_check) { return to_check >= 'a' && to_check <= 'z'; }

int is_num(char to_check) { return to_check >= '0' && to_check <= '9'; }

int isupper_custom(char to_check) { return to_check >= 'A' && to_check <= 'Z'; }

char get_lower(char uppercase) { return uppercase + 32; }

char get_upper(char lowercase) { return lowercase - 32; }
